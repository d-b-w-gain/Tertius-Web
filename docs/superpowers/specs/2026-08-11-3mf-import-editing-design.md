...
An independent API-lifecycle dispatcher claims only rows whose job is still active on the matching execution with `FOR UPDATE SKIP LOCKED`, commits a bounded owner/attempt lease, performs a short committed pre-publish recheck, closes the database transaction, publishes the exact persisted command with a deterministic `Nats-Msg-Id`, and conditionally marks the row sent. No database lock is held across NATS I/O. Expired leases are reclaimable. A crash after publish but before marking sent can republish only the same deterministic message identity, which JetStream deduplicates safely. NATS publication outages leave the import job and pending outbox durable for bounded backoff/retry; they do not roll back an accepted import.

Queued-job reconciliation is scoped to the job's current execution and its matching outbox row. Pending or leased delivery remains queued regardless of the original queue time. After delivery is marked sent, the worker-start lease begins at `sent_at`; a terminal or exhausted delivery fails the current queued job with the fixed retryable `command_dispatch_failed` code. Outbox rows from older executions cannot fail or dispatch a retried job.
