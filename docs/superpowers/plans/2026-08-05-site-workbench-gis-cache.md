...
- [x] Replace the drawer and draggable-inspector experiment after field use: keep every engineering section visible, use a fixed derived-results panel, autosave the complete canonical `tertius_site.py` after edits, and remember per-project map display preferences without mixing them into engineering inputs.
