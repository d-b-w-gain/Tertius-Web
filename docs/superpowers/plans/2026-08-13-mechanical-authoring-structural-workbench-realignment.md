...
- [ ] Write tests for product immutability, catalogue-row digests, component
  instance registration, port handles, and graph serialization.
- [ ] Define the product, procurement, structural, drawing, component, port,
  connection, and fabrication contracts.
- [ ] Define one current compiled-design schema and canonical digest rules.
- [ ] Prove that projection code cannot accept an independently entered product
  number, length, or section record.
- [ ] Document the mechanical-only `design.py` contract and reserved `tertius`
  namespace.
...
- [ ] Start a fresh session before executing `design.py` and always close it in
  `finally`.
- [ ] Require a `model` Build123D root; remove environment-wide shape
  collection.
- [ ] Fail if managed component factories run without an active session.
- [ ] Fail if removed `TERTIUS_*` globals or project-local reserved runtime
  modules exist.
- [ ] Reconcile each registered managed shape exactly once against the final
  model tree and identify unmanaged geometry.
- [ ] Provide `python -m tertius.runner <project>` for the same local lifecycle.
...
- [ ] Refactor catalogue lookup into immutable `ProductDefinition` values.
- [ ] Make one Cee/Zed member call generate geometry, procurement facts,
...
- [ ] Make endpoints determine placement and physical length; treat ordered
  length and explicit fabrication cuts as separate validated facts.
- [ ] Return a Build123D-compatible managed shape usable directly in the final
  compound.
- [ ] Demonstrate that changing `C10019` to another product changes every
...
- [ ] Ensure coincident/touching shapes remain structurally unconnected unless a
  connection object joins them.
- [ ] Keep unvalidated stiffness/capacity visibly unverified rather than
...
- [ ] Generate `compiled_design.json` first, then project every other artifact
  from that frozen graph.
- [ ] Generate procurement rows from component/product/fabrication facts; keep
...
- [ ] Generate GLB/scene nodes containing stable component and product
  references without label matching.
- [ ] Generate bounds and drawing-input artifacts from the same model instance.
- [ ] Validate matching graph/product/component digests across every
  projection.
...
- [ ] Include the selected configuration revision/digest in the compile or
...
- [ ] Block structural approval when required context/evidence is missing.
...
- [ ] Define a bounded `CompileArtifactPayload` with kind, content type,
...
- [ ] Enforce per-artifact and total-result size limits before publish and after
  decode.
- [ ] Persist all artifacts and finish the job in one database transaction.
- [ ] Add uniqueness for one artifact kind per compile revision and expand the
  artifact-kind column for descriptive names.
- [ ] Prune complete bundles, never individual artifacts that would leave a
...
- [ ] Remove request-time procurement AST/GLTF reconstruction and its cache.
- [ ] Remove the Procurement action that asks AI to add `tertius_bom` plumbing
  to `design.py`.
- [ ] Make viewer selection, BoM rows, structural results, and drawing callouts
  navigate through the same component instance IDs.
- [ ] Surface readiness gates and exact component/connection diagnostics.
