...
- [x] Implement `master-[0-9]+-[0-9]+-[a-f0-9]{7}` validation and exact single replacement for `$imagepromoter` markers `tertius-api`, `tertius-pi-agent`, and `tertius-ui`.
- [x] Make writes atomic with a sibling temporary file and preserve all unrelated text.
- [x] Replace the six Flux image-policy comments with three CI image-promoter tag markers; repository values remain unmarked.
