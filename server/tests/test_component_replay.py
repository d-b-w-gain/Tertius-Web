...
"""
    ...
    definition_tree = ast.parse(definition_source)
    library_assignment = next(
        node
        for node in definition_tree.body
        if isinstance(node, ast.Assign)
        and any(isinstance(target, ast.Name) and target.id == "_LIBRARY_FILE" for target in node.targets)
    )
    assert ast.dump(library_assignment.value) == ast.dump(ast.parse('Path("catalog.json")', mode="eval").body)
