...
def test_postgres_test_image_defaults_to_docker_hub(monkeypatch):
    ...
    assert postgres_test_image() == "postgres:16"


def test_postgres_test_image_can_be_overridden(monkeypatch):
    monkeypatch.setenv("POSTGRES_TEST_IMAGE", "public.ecr.aws/docker/library/postgres:16")

    assert postgres_test_image() == "public.ecr.aws/docker/library/postgres:16"
