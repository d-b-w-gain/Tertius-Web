...
def _has_named_group_child_with_meshes(node: dict[str, Any]) -> bool:
    return any(
        (
            _is_named_group(child)
            and not _is_generated_or_default(_node_name(child))
            and _has_mesh_descendant(child)
        )
        ...
        if _is_named_group(node) and _has_mesh_descendant(node):
            if _has_named_group_child_with_meshes(node):
                ...
                components.append({
                    ...
                    "visual_instance_count": len(visual_node_ids) if len(visual_node_ids) > 1 else None,
                })
