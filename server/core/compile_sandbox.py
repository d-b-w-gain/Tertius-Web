...
BOM_MANIFEST_FILENAME = "tertius-bom-manifest.json"
...
bom_manifest_path = project_dir / "tertius-bom-manifest.json"
...
try:
    ...
    from tertius_bom import get_manifest as get_tertius_bom_manifest

    bom_manifest_path.write_text(
        json.dumps(
            get_tertius_bom_manifest(),
            sort_keys=True,
            separators=(",", ":"),
            ensure_ascii=False,
            allow_nan=False,
    ...
    def append_root_shape(value):
        if isinstance(value, bd.Shape) and hasattr(value, "volume"):
            geom_type = value.geom_type() if callable(value.geom_type) else value.geom_type
    ...
    if not shapes:
        for name, val in env.items():
            ...
            for attribute in ("shape", "part"):
                target = getattr(val, attribute, None)
                if isinstance(target, bd.Shape):
                    append_root_shape(target)
                    break
    ...
    visual_bom_by_label = {}
    visited_bom_containers = set()

    def remember_visual_bom(value):
        if isinstance(value, dict):
            container_id = id(value)
            if container_id in visited_bom_containers:
                return
            visited_bom_containers.add(container_id)
            for item in value.values():
                remember_visual_bom(item)
            return
        ...
        for attribute in ("shape", "part"):
            ...
            if isinstance(target, bd.Shape):
                remember_visual_bom(target)
                return
        if not isinstance(value, bd.Shape):
            return
        for node in bd.PreOrderIter(value):
            metadata = getattr(node, "tertius_bom", None)
            label = getattr(node, "label", "")
            if metadata is not None and label:
                visual_bom_by_label[label] = metadata

    for name, val in env.items():
        if str(name).startswith("__"):
            continue
        remember_visual_bom(val)
    ...
    for shape in final_shapes:
        for node in bd.PreOrderIter(shape):
            metadata = getattr(node, "tertius_bom", None)
            label = getattr(node, "label", "")
            if metadata is not None and label:
                visual_bom_by_label[label] = metadata
    ...
    if structural_authoring.get("mode") == "generated":
        ...
        def collect_container_shapes(value):
            ...
            for attribute in ("shape", "part"):
                target = getattr(value, attribute, None)
                if isinstance(target, bd.Shape):
                    collect_container_shapes(target)
                    return
        ...
        if export_format in ("gltf", "glb"):
            try:
                ...
                def color_factor(color):
                    ...
                    try:
                        if isinstance(color, bd.Color):
                            rgba = list(color)
                            rgba[:3] = [
                                srgb_to_linear_float32(float(component))
                ...
                for node in bd.PreOrderIter(compound):
                    ...
                    if node.label or node.color is not None or bom_metadata is not None:
                        ...
                        if not inst_label.IsNull():
                            ...
                            if bom_metadata is not None:
                                ...
                                if node.label:
                                    label_to_bom[node.label] = bom_metadata
                ...
                def patch_gltf_json(gltf_json, name_mapping, color_mapping, bom_mapping, label_bom_mapping, visual_metadata):
                    ...
                    for node in gltf_json.get("nodes", []):
                        ...
                        if node_tag in name_mapping:
                            ...
                            if node["name"] in label_bom_mapping:
                                extras = node.setdefault("extras", {})
                                extras["tertiusBom"] = label_bom_mapping[node["name"]]
                            changed = True
                ...
                def patch_glb_metadata(glb_path, name_mapping, color_mapping, bom_mapping, label_bom_mapping):
                    ...
                    changed = patch_gltf_json(gltf_json, name_mapping, color_mapping, bom_mapping, label_bom_mapping, visual_tree)
                ...
                def patch_gltf_metadata(gltf_path, name_mapping, color_mapping, bom_mapping, label_bom_mapping):
                ...
                if export_format == "glb":
                    patch_glb_metadata(str(output_path), tag_to_name, tag_to_color, tag_to_bom, label_to_bom)
                else:
                    patch_gltf_metadata(str(output_path), tag_to_name, tag_to_color, tag_to_bom, label_to_bom)
...
class CompileSandboxResult:
    ...
    bom_manifest_path: Path | None = None
...
def run_compile_sandbox(project_dir: Path, export_format: str, quality: str | None = None, timeout_seconds: int = 30) -> CompileSandboxResult:
    ...
    return CompileSandboxResult(
        ...
        bom_manifest_path=(
            project_dir / BOM_MANIFEST_FILENAME
            if (project_dir / BOM_MANIFEST_FILENAME).is_file()
            else None
        ),
    )
